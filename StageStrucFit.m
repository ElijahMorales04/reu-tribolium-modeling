...
% disp(['m_e = ', num2str(m_e)]);
% disp(['m_l = ', num2str(m_l)]);
% disp(['m_ = ', num2str(m_p)]);
% disp(['m_a = ', num2str(m_a)]);
...
plot(timeData, larvaeData,'square','MarkerFaceColor', 'b','MarkerEdgeColor','b');
plot(timeData, pupaeData,'diamond','MarkerFaceColor', 'r','MarkerEdgeColor','r');
plot(timeData, adultsData,'^','MarkerFaceColor', 'k','MarkerEdgeColor','k');
...
function LPAMat = simulate_population(params, MaxN)
    ...
    for n = 1:MaxN
        ...
        for i = 3:29
            ...
            elseif i == 29
                ProjMatrix(i, i-1) = (1 - mu_a) *  P(29, n) + P(28, n) * exp(-c_pa * P(29, n)) ;
            end
        end
