...
for i = 2:MaxT+1
    ...
    DF(i,4) = DF(i-1,3)*exp(-c_pa*DF(i-1,4))+((1-mu_a)*(1-gamma))*DF(i-1,4)+gamma*DF5(i-1,4)*(1-mu_a)*(1-Eps);
    
    DF2(i,2)= b*DF2(i-1,4)*exp(-c_ea*DF2(i-1,4)-c_el*DF2(i-1,2));
    DF2(i,3)= (1-mu_l)*DF2(i-1,2);
    DF2(i,4) = DF2(i-1,3)*exp(-c_pa*DF2(i-1,4))+((1-mu_a)*(1-gamma))*DF2(i-1,4)+(DF5(i-1,4))*gamma*(1-mu_a)*(1-Eps);
    
    DF3(i,2)= b*DF3(i-1,4)*exp(-c_ea*DF3(i-1,4)-c_el*DF3(i-1,2));
    DF3(i,3)= (1-mu_l)*DF3(i-1,2);
    DF3(i,4) = DF3(i-1,3)*exp(-c_pa*DF3(i-1,4))+((1-mu_a)*(1-gamma))*DF3(i-1,4)+(DF5(i-1,4))*gamma*(1-mu_a)*(1-Eps);
    
    DF4(i,2)= b*DF4(i-1,4)*exp(-c_ea*DF4(i-1,4)-c_el*DF4(i-1,2));
    DF4(i,3)= (1-mu_l)*DF4(i-1,2);
    DF4(i,4) = DF4(i-1,3)*exp(-c_pa*DF4(i-1,4))+((1-mu_a)*(1-gamma))*DF4(i-1,4)+(DF5(i-1,4))*gamma*(1-mu_a)*(1-Eps);
    
    DF5(i,2)= b*DF5(i-1,4)*exp(-c_ea*DF5(i-1,4)-c_el*DF5(i-1,2));
    DF5(i,3)= (1-mu_l)*DF5(i-1,2);
    DF5(i,4) = DF5(i-1,3)*exp(-c_pa*DF5(i-1,4))+((1-mu_a)*(1-gamma))*DF5(i-1,4)+(1/4*DF(i-1,4)+1/4*DF2(i-1,4)+1/4*DF3(i-1,4)+1/4*DF4(i-1,4))*gamma*(1-mu_a)*(1-Eps);
end
