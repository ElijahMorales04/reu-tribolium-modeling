...
for i = 2:MaxT+1
    ...
    DF1(i,4) = DF1(i-1,3)*exp(-c_pa*(DF1(i-1,4)+DF1M(i-1,4)))+((1-mu_a)*(1-gammaR))*DF1(i-1,4)+1/2*(DF2(i-1,4)+DF3(i-1,4))*gammaR*(1-mu_a)*(1-Eps);
    
    DF2(i,2)= b*DF2(i-1,4)*exp(-c_ea*(DF2(i-1,4)+DF2M(i-1,4))-c_el*(DF2(i-1,2)+DF2M(i-1,2)));
    DF2(i,3)= (1-mu_l)*DF2(i-1,2);
    DF2(i,4) = DF2(i-1,3)*exp(-c_pa*(DF2(i-1,4)+DF2M(i-1,4)))+((1-mu_a)*(1-gammaR))*DF2(i-1,4)+1/2*(DF4(i-1,4)+DF1(i-1,4))*gammaR*(1-mu_a)*(1-Eps);
    
    DF3(i,2)= b*DF3(i-1,4)*exp(-c_ea*(DF3(i-1,4)+DF3M(i-1,4))-c_el*(DF3(i-1,2)+DF3M(i-1,2)));
    DF3(i,3)= (1-mu_l)*DF3(i-1,2);
    DF3(i,4) = DF3(i-1,3)*exp(-c_pa*(DF3(i-1,4)+DF3M(i-1,4)))+((1-mu_a)*(1-gammaR))*DF3(i-1,4)+1/2*(DF1(i-1,4)+DF4(i-1,4))*gammaR*(1-mu_a)*(1-Eps);
    
    DF4(i,2)= b*DF4(i-1,4)*exp(-c_ea*(DF4(i-1,4)+DF4M(i-1,4))-c_el*(DF4(i-1,2)+DF4M(i-1,2)));
    DF4(i,3)= (1-mu_l)*DF4(i-1,2);
    DF4(i,4) = DF4(i-1,3)*exp(-c_pa*(DF4(i-1,4)+DF4M(i-1,4)))+((1-mu_a)*(1-gammaR))*DF4(i-1,4)+1/2*(DF3(i-1,4)+DF2(i-1,4))*gammaR*(1-mu_a)*(1-Eps);
    
    
    DF1M(i,2)= b*DF1M(i-1,4)*exp(-c_ea*(DF1(i-1,4)+DF1M(i-1,4))-c_el*(DF1(i-1,2)+DF1M(i-1,2)));
    DF1M(i,3)= (1-mu_l)*DF1M(i-1,2);
    DF1M(i,4) = DF1M(i-1,3)*exp(-c_pa*(DF1(i-1,4)+DF1M(i-1,4)))+((1-mu_a)*(1-gammaM))*DF1M(i-1,4)+1/2*(DF2M(i-1,4)+DF3M(i-1,4))*gammaM*(1-mu_a)*(1-Eps);
    
    DF2M(i,2)= b*DF2M(i-1,4)*exp(-c_ea*(DF2(i-1,4)+DF2M(i-1,4))-c_el*(DF2(i-1,2)+DF2M(i-1,4)));
    DF2M(i,3)= (1-mu_l)*DF2M(i-1,2);
    DF2M(i,4) = DF2M(i-1,3)*exp(-c_pa*(DF2(i-1,4)+DF2M(i-1,4)))+((1-mu_a)*(1-gammaM))*DF2M(i-1,4)+1/2*(DF4M(i-1,4)+DF1M(i-1,4))*gammaM*(1-mu_a)*(1-Eps);
    
    DF3M(i,2)= b*DF3M(i-1,4)*exp(-c_ea*(DF3(i-1,4)+DF3M(i-1,4))-c_el*(DF3(i-1,2)+DF3M(i-1,2)));
    DF3M(i,3)= (1-mu_l)*DF3M(i-1,2);
    DF3M(i,4) = DF3M(i-1,3)*exp(-c_pa*(DF3(i-1,4)+DF3M(i-1,4)))+((1-mu_a)*(1-gammaM))*DF3M(i-1,4)+1/2*(DF1M(i-1,4)+DF4M(i-1,4))*gammaM*(1-mu_a)*(1-Eps);
    
    DF4M(i,2)= b*DF4M(i-1,4)*exp(-c_ea*(DF4(i-1,4)+DF4M(i-1,4))-c_el*(DF4(i-1,2)+DF4M(i-1,2)));
    DF4M(i,3)= (1-mu_l)*DF4M(i-1,2);
    DF4M(i,4) = DF4M(i-1,3)*exp(-c_pa*(DF4(i-1,4)+DF4M(i-1,4)))+((1-mu_a)*(1-gammaM))*DF4M(i-1,4)+1/2*(DF3M(i-1,4)+DF2M(i-1,4))*gammaM*(1-mu_a)*(1-Eps);
end
